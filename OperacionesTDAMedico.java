public class OperacionesTDAMedico 
{
    Medico medico;
    NodoMedico primero, ultimo, aux, aux2, nuevo;
    int cantNodos = 0;
    ...
    public void inserta(Medico medico, int pos) {
        nuevo = new NodoMedico(medico);
    ...
    public String imprimirListaMedico() {
        String mensaje = "";
        int i = 1;
        if (!esVacia()) {
            ...
            aux = primero;
            while (aux != null) {
                mensaje += "Posicion: " + i + " - Valor: " + aux.getMedico() + "\n";
    ...
    public Medico primero() 
    {
        if (!esVacia()) 
        {
            return primero.getMedico();
            
        } 
        else
        {
            System.err.println("ENTRO EN EL METODO MEDICO PRIMERO, LA LISTA ESTA VACIA");
            return null;
        }
