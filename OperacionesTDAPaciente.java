public class OperacionesTDAPaciente 
{
    
    Paciente paciente;
    NodoPaciente primero, ultimo, aux, aux2, nuevo;
    int cantNodos = 0;
...
//de la posiciÃ³n p y siguientes a la posiciÃ³n inmediatamente posterior. 	
    ...
    public void inserta(Paciente paciente, int pos) {
        nuevo = new NodoPaciente(paciente);
    ...
    public String imprimirListaPaciente() {
        String mensaje = "";
        int i = 1;
        if (!esVacia()) {
            ...
            aux = primero;
            while (aux != null) {
                mensaje += "Posicion: " + i + " - Valor: " + aux.getPaciente() + "\n";
    ...
    public Paciente primero() 
    {
        if (!esVacia()) 
        {
            return primero.getPaciente();
            
        } 
        else
        {
            System.err.println("ENTRO EN EL METODO PACIENTE PRIMERO, LA LISTA ESTA VACIA");
            return null;
        }
